...
const METHOD_KEYS = ["m1", "m2", "m3"] as const;
const INTEGRATION_KEYS = ["i1", "i2", "i3", "i4"] as const;
const FAQ_KEYS = ["q1", "q2", "q3"] as const;

export function ControlHorarioContent() {
  const t = useTranslations("satellite.controlHorario");
      ...
      <main className="flex-1">
        ...
        <section className="py-20 bg-background">
          <div className="container mx-auto px-4 max-w-4xl">
            <h2 className="text-3xl font-bold text-center mb-4">{t("checkinMethods.title")}</h2>
            <p className="text-muted-foreground text-center mb-12">{t("checkinMethods.subtitle")}</p>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
              {METHOD_KEYS.map((k) => (
                <div key={k} className="rounded-xl border bg-card p-6 space-y-3">
                  <span className="text-xs font-medium px-2 py-1 rounded-full bg-primary/10 text-primary">{t(`checkinMethods.items.${k}.badge`)}</span>
                  <h3 className="font-semibold text-lg">{t(`checkinMethods.items.${k}.title`)}</h3>
                  <p className="text-sm text-muted-foreground">{t(`checkinMethods.items.${k}.desc`)}</p>
                </div>
              ))}
            </div>
          </div>
