...
export function CrmContent() {
  const t = useTranslations("satellite.crm");
      ...
      <main className="flex-1">
        ...
        <section className="py-20 bg-muted/30">
          <div className="container mx-auto px-4 max-w-3xl">
            ...
            <p className="text-muted-foreground text-center mb-10 max-w-xl mx-auto">{t("why.subtitle")}</p>
            <div className="space-y-4">
              {WHY_KEYS.map((k) => (
                <div key={k} className="rounded-xl border bg-background p-5">
                  <h3 className="font-semibold mb-1">{t(`why.items.${k}.title`)}</h3>
                  <p className="text-sm text-muted-foreground">{t(`why.items.${k}.desc`)}</p>
                </div>
              ))}
            </div>
          </div>
