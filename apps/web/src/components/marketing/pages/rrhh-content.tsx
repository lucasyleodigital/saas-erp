...
const COMPLIANCE_KEYS = ["c1", "c2", "c3"] as const;

export function RrhhContent() {
  const t = useTranslations("satellite.rrhh");
      ...
      <main className="flex-1">
        ...
        <section className="py-20 bg-muted/30">
          <div className="container mx-auto px-4 max-w-3xl">
            <h2 className="text-3xl font-bold text-center mb-4">{t("compliance.title")}</h2>
            ...
            <div className="space-y-4">
              {COMPLIANCE_KEYS.map((k) => (
                <div key={k} className="rounded-xl border bg-background p-5">
                  <h3 className="font-semibold mb-1">{t(`compliance.items.${k}.title`)}</h3>
                  <p className="text-sm text-muted-foreground">{t(`compliance.items.${k}.desc`)}</p>
                </div>
              ))}
            </div>
          </div>
