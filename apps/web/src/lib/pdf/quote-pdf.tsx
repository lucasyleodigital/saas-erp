...
const s = StyleSheet.create({
  ...
  tRow:        { flexDirection: "row", width: 190, justifyContent: "space-between", paddingVertical: 4, paddingHorizontal: 10 },
  tLbl:        { fontSize: 9, color: "#6b7280" },
  tVal:        { fontSize: 9, color: "#111827", fontFamily: "Helvetica-Bold" },
  tRowFinal:   { flexDirection: "row", width: 190, justifyContent: "space-between", paddingVertical: 8, paddingHorizontal: 10, backgroundColor: PRIMARY, borderRadius: 4, marginTop: 4 },
  tLblFinal:   { fontSize: 11, color: "#fff", fontFamily: "Helvetica-Bold" },
  tValFinal:   { fontSize: 11, color: "#fff", fontFamily: "Helvetica-Bold" },
  validity:    { marginBottom: 16, padding: 12, backgroundColor: "#eff6ff", borderRadius: 4, borderLeftWidth: 3, borderLeftColor: PRIMARY, borderLeftStyle: "solid" },
  validityTxt: { fontSize: 8.5, color: "#1e40af" },
...
const STATUS_COLOR: Record<string, string> = { DRAFT: "#6b7280", SENT: "#2563eb", ACCEPTED: "#059669", REJECTED: "#dc2626", EXPIRED: "#d97706" };
      ...
      <Page size="A4" style={s.page}>
        ...
        <View style={s.header}>
          ...
          <View>
            ...
            <Text style={s.docNum}>{quote.number}</Text>
            <View style={[s.badge, { backgroundColor: STATUS_COLOR[quote.status] ?? "#6b7280" }]}>
              <Text style={s.badgeTxt}>{STATUS_LABEL[quote.status] ?? quote.status}</Text>
            </View>
          </View>
        </View>
        ...
        <View style={s.infoRow}>
          <View style={s.infoBlock}>
            <Text style={s.infoLabel}>Presupuesto para</Text>
          ...
          <View style={s.infoBlock}>
            ...
            <Text style={s.infoTxt}>Fecha: {fmtDate(quote.issueDate)}</Text>
            {quote.validUntil && <Text style={s.infoTxt}>Válido hasta: {fmtDate(quote.validUntil)}</Text>}
        ...
        {quote.validUntil && (
          <View style={s.validity}>
            <Text style={s.validityTxt}>
              Este presupuesto es válido hasta el {fmtDate(quote.validUntil)}.
              Para aceptarlo, responda a este documento o contacte con nosotros.
            </Text>
          </View>
        )}
        ...
        <View style={s.totals}>
          <View style={s.tRow}>
            ...
            <Text style={s.tVal}>{fmt(Number(quote.subtotal), cur)}</Text>
          </View>
          {Number(quote.taxAmount) > 0 && (
            <View style={s.tRow}>
              <Text style={s.tLbl}>IVA (21%)</Text>
              <Text style={s.tVal}>{fmt(Number(quote.taxAmount), cur)}</Text>
            </View>
          )}
          <View style={s.tRowFinal}>
            <Text style={s.tLblFinal}>TOTAL</Text>
            <Text style={s.tValFinal}>{fmt(Number(quote.total), cur)}</Text>
          </View>
        </View>
