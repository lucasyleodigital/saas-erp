...
export class BankService {
  ...
  private parseStatement(buffer: Buffer): Array<{ date: Date; amount: number; description: string; reference: string }> {
    ...
    const DATE_KEYS = ["fecha", "date", "fecha valor", "fecha operacion", "value date"];
    const DESC_KEYS = ["concepto", "description", "descripcion", "detalle", "movimiento", "referencia"];
    const REF_KEYS = ["referencia", "reference", "ref", "numero"];
    ...
    return rows.map((r) => {
      ...
      if (dateKey && r[dateKey]) {
        if (r[dateKey] instanceof Date) {
          date = r[dateKey];
        } else {
          const s = String(r[dateKey]).trim();
          const dmY = s.match(/^(\d{1,2})[\/\-](\d{1,2})[\/\-](\d{4})$/);
          if (dmY) date = new Date(`${dmY[3]}-${dmY[2]!.padStart(2, "0")}-${dmY[1]!.padStart(2, "0")}`);
